...
\newcommand{\hrefwrpc}[1]{
  \tts{\href{https://gitlab.com/ohwr/project/wr-cores/blob/\wrpcrelease/#1}{#1}}}
...
available at \url{https://gitlab.com/ohwr/project/wr-cores/wikis/Documents} for officially
...
\item \url{https://gitlab.com/ohwr/project/wr-cores/wikis/Documents}

  Hosts the pdf documentation for every official release.
  The place where you can find already synthesized demo bitstreams,
  ready to be downloaded to one of the officially supported boards.

\item \url{https://gitlab.com/ohwr/project/wr-cores.git}

  Repository with HDL sources of the WRPC.

\item \url{https://gitlab.com/ohwr/project/wrpc-sw.git}
...
\item \url{https://gitlab.com/ohwr/project/hdl-make.git}

  \textit{hdlmake} is used in the HDL synthesis process to create a Makefile and
  Xilinx ISE / Altera Quartus project file

% Split url into two \href
\item {\href{https://gitlab.com/ohwr/project/wrpc-sw/wikis/uploads/9f9224d2249848ed3e854636de9c08dc/riscv-11.2-small.tgz}{https://gitlab.com/ohwr/project/wrpc-sw/wikis/uploads/}\\
\href{https://gitlab.com/ohwr/project/wrpc-sw/wikis/uploads/9f9224d2249848ed3e854636de9c08dc/riscv-11.2-small.tgz}{9f9224d2249848ed3e854636de9c08dc/riscv-11.2-small.tgz}}
...
can be chosen:
\begin{itemize}
\item \href{https://gitlab.com/ohwr/project/spec/wikis}{SPEC PCIe board} +
  \href{https://gitlab.com/ohwr/project/fmc-dio-5chttla/wikis}{FMC DIO card} + PC
  computer running Linux
\item \href{https://gitlab.com/ohwr/project/svec/wikis}{SVEC VME board} + VME crate with
  a single board computer running Linux\footnote{\label{note_a20}In our test setup
    we used MEN A20 board}
\item \href{https://gitlab.com/ohwr/project/vfc-hd/wikis}{VFC-HD VME board} +
  \href{https://gitlab.com/ohwr/project/fmc-dio-5chttla/wikis}{FMC DIO card} +
  VME crate with a single board computer running Linux\footref{note_a20}
\item \href{https://gitlab.com/ohwr/project/fasec/wikis}{FASEC board} +
  \href{https://gitlab.com/ohwr/project/fmc-dio-5chttla/wikis}{FMC DIO card}
\end{itemize}
...
\begin{itemize}
  ...
  \item a pair of WR-supported SFP transceivers\footnote{The list of supported
    ...
    \url{https://gitlab.com/ohwr/project/white-rabbit/wikis/SFP}}
...
available from \textit{https://gitlab.com/ohwr}.
...
software running inside the core. If, for some reasons, you need to modify
the soft-core processor's software, please check also
...
Please clone the \textit{hdlmake} repository from its location given in
section \ref{Repositories and Releases}:
\begin{lstlisting}
$ cd <your-location>
$ git clone https://gitlab.com/ohwr/project/hdl-make.git
$ cd hdl-make
$ pip install .
\end{lstlisting}

The last command install \textit{hdlmake} system wide.  You might prefer
...
\texttt{<your-location>/wr-cores/ip\_cores}
...
\textit{https://gitlab.com/ohwr} or
...
\begin{lstlisting}
$ wget https://gitlab.com/ohwr/project/wrpc-sw/wikis/uploads/\
9f9224d2249848ed3e854636de9c08dc/riscv-11.2-small.tgz
...
\begin{lstlisting}
$ git clone https://gitlab.com/ohwr/project/wrpc-sw.git <your_location>/wrpc-sw
...
\begin{itemize*}
  \item \textit{wrc.bin} can be then used with the loader from \textit{spec-sw}
    or \textit{svec-sw} software package to program the soft-core processor
    inside the White
    Rabbit PTP Core (see section \ref{Programming FPGA} for details).
  \item \textit{wrc.bram} you can use to initialize WRPC internal RAM with
    software during the synthesis for Xilinx FPGAs.
  \item \textit{wrc.mif} you can use to initialize WRPC internal RAM with
...
(\textit{spec-sw}) from \textit{https://gitlab.com/ohwr}.
It is a set of Linux kernel drivers and
userspace tools, that interacts with a SPEC board plugged into a PCI-Express
slot.\\

\begin{lstlisting}
$ git clone https://gitlab.com/ohwr/project/spec-sw.git <your_location>/spec-sw
...
\begin{lstlisting}
$ wget https://gitlab.com/ohwr/project/spec-sw/wikis/uploads/\
636e7eaa2e1c3a884e2f9f694fdfd1b9/spec-init.bin-2015-09-18
...
compiled in section \ref{Software compilation}, you can use the
\texttt{spec-cl} tool:
\begin{lstlisting}
...
\end{lstlisting}
From version 4.3, it is possible to trigger creation of SDBFS via SNMP.
For more details please refer to section~\ref{Generation of SFBFS via SNMP}.
...
the defaults for wrpc-v4.1 release bitstream available on
\textit{https://gitlab.com/ohwr},
...
(\url{https://gitlab.com/ohwr/project/white-rabbit/wikis/Calibration}). If you
...
(\url{https://gitlab.com/ohwr/project/white-rabbit/wikis/Documents/White-Rabbit-calibration-procedure}).\\
...
startup. If you run WRPC release v4.1 or higher, the default software
...
but in a single line, which is easier to parse and analyze. Example for the slave mode:
...
\begin{lstlisting}
wrc# stat
lnk:1 rx:10955 tx:8050 lock:1 ptp:master sec:1630462668 nsec:109213512 hd:33525 \
md:30000 ad:0 temp:48.3125 C
lnk:1 rx:10956 tx:8054 lock:1 ptp:master sec:1630462669 nsec:109084328 hd:33523 \
md:30000 ad:0 temp:48.3125 C
(...)
\end{lstlisting}

\vspace{1em}
Unlike \texttt{gui}, the \texttt{stat} command runs in the background: you can still
...
are reported once per second the node is running in master and GrandMaster mode.
...
wrc#
\end{lstlisting}
At the same time the following will be printed on the serial console:
\begin{lstlisting}
wrc# temp
pcb:46.3750
wrc# mode
...
netconsole disabled
\end{lstlisting}
To make netconsole to listen for incoming messages please execute the following
command:
\begin{lstlisting}
wrc# netconsole wait
netconsole is waiting for peer
...
% ==========================================================================
\subsubsection{Setting up init script via SNMP}
\label{Setting up init script via SNMP}

When options \texttt{CONFIG\_SNMP\_INIT}, \texttt{CONFIG\_FLASH\_INIT}
and \texttt{CONFIG\_SNMP\_SET} are enabled in the dot-config, it is possible to
add a command at the end of an \textit{init script}. For more information about
the~\textit{init script} please refer to section~\ref{Init script} and
description of an \texttt{init} command in
\hyperlink{cmd init add}{appendix~\ref{WRPC Shell commands}}.

To add a command at the end of an init script, first specify a command by
filling OID \texttt{wrpcInitScriptConfigLine.0}, then trigger the write to
a Flash by setting \texttt{writeToFlash} to
OID \texttt{wrpcInitScriptConfigLine.0}.

The following example adds \texttt{my\_init\_command} at the end of
the init script:
\begin{lstlisting}
$ snmpset $SNMP_OPT wrpcInitScriptConfigLine.0 = "my_init_command"
WR-WRPC-MIB::wrpcInitScriptConfigLine.0 = STRING: my_init_command
$ snmpset $SNMP_OPT wrpcInitScriptConfigApply.0 = writeToFlash
WR-WRPC-MIB::wrpcInitScriptConfigApply.0 = INTEGER: applySuccessful(100)
\end{lstlisting}

To erase the init script please write \texttt{eraseFlash} to
OID \texttt{wrpcInitScriptConfigApply.0}. For example:
\begin{lstlisting}
$ snmpset $SNMP_OPT wrpcInitScriptConfigApply.0 = eraseFlash
WR-WRPC-MIB::wrpcInitScriptConfigApply.0 = INTEGER: applySuccessful(100)
\end{lstlisting}
As in version 4.3, there is no way to dump init scrips via SNMP.

% ==========================================================================
\subsubsection{Generation of SFBFS via SNMP}
\label{Generation of SFBFS via SNMP}
When options \texttt{CONFIG\_SNMP\_SDB}, \texttt{CONFIG\_GENSDBFS}
and \texttt{CONFIG\_SNMP\_SET}
are enabled in the dot-config, it is possible to trigger creation and erase
of SDBFS image in a Flash via SNMP. The final result is the same as executing
a command \hyperlink{cmd sdb fs}{\texttt{sdb~fs~...}} for creation or
\hyperlink{cmd sdb fse}{\texttt{sdb~fse~...}} for erase from a serial console.

To create SDBFS image in the flash, please set first parameters of used Flash:
\begin{lstlisting}
$ snmpset $SNMP_OPT wrpcSdbMemType.0 = 0
WR-WRPC-MIB::wrpcSdbMemType.0 = INTEGER: flash(0)
$ snmpset $SNMP_OPT wrpcSdbBaseAddr.0 = 3014656
WR-WRPC-MIB::wrpcSdbBaseAddr.0 = INTEGER: 3014656
$ snmpset $SNMP_OPT wrpcSdbParam.0 = 64
WR-WRPC-MIB::wrpcSdbParam.0 = INTEGER: 64
\end{lstlisting}
For details about \texttt{memtype}, \texttt{baseadr} and \texttt{param}, please
refer to a description of a command
\hyperlink{cmd sdb fs}{\texttt{sdb fs <memtype> <baseadr> <param>}}
in the~\hyperlink{cmd sdb fs}{appendix~\ref{WRPC Shell commands}}.

Then trigger the creation itself:
\begin{lstlisting}
$ snmpset -t 20 $SNMP_OPT wrpcSdbApply.0 = writeToFlash
WR-WRPC-MIB::wrpcSdbApply.0 = INTEGER: applySuccessful(100)
\end{lstlisting}
When creating or erasing the Flash, it is important to set timeout longer than
usual (in this case 20 seconds, parameter \texttt{-t 20} of \texttt{snmpset}),
otherwise \texttt{snmpset} will resend request to write to memory.

The way to erase flash is very similar to the creation:
\begin{lstlisting}
$ snmpset $SNMP_OPT wrpcSdbMemType.0 = 0
WR-WRPC-MIB::wrpcSdbMemType.0 = INTEGER: flash(0)
$ snmpset $SNMP_OPT wrpcSdbBaseAddr.0 = 3014656
...
$ snmpset $SNMP_OPT wrpcSdbParam.0 = 64
WR-WRPC-MIB::wrpcSdbParam.0 = INTEGER: 64
$ snmpset -t 20 $SNMP_OPT wrpcSdbApply.0 = eraseFlash
WR-WRPC-MIB::wrpcSdbApply.0 = INTEGER: applySuccessful(100)
\end{lstlisting}
...
you have not yet cloned it, please see section \ref{Software compilation}
...
For the \textit{spec} board and WRPC using RISC-V as a soft-core processor,
the procedure to read the WRPC memory is different. The read can be done by
...
that the read of memory using spec-cl causes the restart of the RISC-V
soft-core.

\textbf{Note:} Depending on the selected soft-core processor architecture
the \texttt{wrpc-dump} can be built for LM32 or RISC-V. The tool does not
support both architectures in the same binary.
...
at binary address 0xa0. It can be queried by \textit{etherbone},
seen in memory maps or displayed with \texttt{uptime} command.
...
time. The following Kconfig options are available in the software:
\begin{longtable}{  p{6.5cm}  p{9cm} }
...
\small{\textit{CONFIG\_VLAN\_NR}} &
  ...
  The default \textit{vlan} number for the CPU (class 0).  All network traffic
        directed to (and originating from) the soft-core processor will
...
at: \url{https://gitlab.com/ohwr/project/white-rabbit/wikis/Documents/White-Rabbit-calibration-procedure}}.
...
tools used to build and run it, you can write at our forums
...
Please note that some commands are available only when enabled in Kconfig during
the build of WRPC.

\renewcommand{\descriptionlabel}[1]{\texttt{#1}}

\begin{description}
\item[calibration] (legacy)

  Tries to read t2/4 phase transition value from
...
\item[config] \mbox{}
  ...
  Prints the dot-config file used to build this instance of
  WRPC and ppsi. It is an optional command, enabled at build time by
  \texttt{CONFIG\_CMD\_CONFIG}.

\item[daclog off]
\item[daclog <ipaddr> <macaddr>] \mbox{}

  Sends the \textit{DAC} output and input to
  the \textit{VCXO} main oscillator via UDP packets to port 1050 on
  a host defined with \code{<ipaddr>} and \code{<macaddr>}.
  It is an optional command, enabled at build time by \texttt{CONFIG\_DACLOG}.

\item[delays]
\item[delays <tx> <rx>] \mbox{}

  Gets or sets the constant delays in frame
  transmission, only available if \texttt{CONFIG\_CMD\_LL}. Delays are
  expressed in picoseconds. This command changes rx and tx deltas for
  the currently matched SFP and both, \code{delta\_rxm} and \code{delta\_txm}.
  For master mode, command has to be executed before the peer enters
  CALIBRATED state. Otherwise it makes no effect on slave side. For slave mode,
  if the command is executed before CALIBRATED is reached, then both slave
  deltas are changed \code{delta\_rxs} (with added bitslide) and
  \code{delta\_txs}. After the state CALIBRATED is reached, at every
  executrion of \code{delays} command both master deltas (\code{delta\_rxm}
  and \code{delta\_txm}) are be updated in realtime.
  If the SFP match is performed (e.g. link restart) the \code{delays} command
  has to be executed once more.

\item[devmem <addr>]
\item[devmem <addr> <value>] \mbox{}

  Reads or writes a 32-bit word from memory
  and/or FPGA registers. Only available if \texttt{CONFIG\_CMD\_LL} is set.

\item[diag] \mbox{}

  Handle diagnostics for auxiliary user registers in HDL. Only available
  if \texttt{CONFIG\_AUX\_DIAG}.

\item[diag ro <reg\_no>] \mbox{}

  Reads register <reg\_no> from read-only bank.

\item[diag rw <reg\_no>] \mbox{}

  Reads register <reg\_no> from read-write bank.

\item[diag w <reg\_no>] \mbox{}

  Writes register <reg\_no> to read-write bank.

\item[ep link] \mbox{}

  Display link status. Available if \texttt{CONFIG\_CMD\_EP} is set.

\item[ep rd <REG>] \mbox{}

  Read a register from the endpoint. Available if
  \texttt{CONFIG\_CMD\_EP} is set.

\item[ep autoneg restart] \mbox{}
\item[ep autoneg off] \mbox{}
\item[ep autoneg on] \mbox{}

  Control endpoint autonegotiation. Available if
  \texttt{CONFIG\_CMD\_EP} is set.

\item[ep stat] \mbox{}

  Display endpoint status. Available if
  \texttt{CONFIG\_CMD\_EP} is set.

\item[ep tx 0|1] \mbox{}

  Enable or disable the SFP tx (drives the \texttt{tx\_disable} line).
  Available if \texttt{CONFIG\_CMD\_EP} is set.

\item[ep reset] \mbox{}

  Put the endpoint on reset.
  Available if \texttt{CONFIG\_CMD\_EP} is set.

\item[ep pd] \mbox{}

  Immediately put the endpoint on powerdown. Software may not see the loss
  of sync.
  Available if \texttt{CONFIG\_CMD\_EP} is set.
...
\item[ep en 0|1] \mbox{}
  ...
  Enable or disable the endpoint. Without argument, the command enables the
  endpoint. Enabling will bring the endpoint out of reset.
  Available if \texttt{CONFIG\_CMD\_EP} is set.

\item[faketemp]
\item[faketemp <T1> <T2> <T3>] \mbox{}

  Reads or sets the three fake temperatures,
...
\item[help] \mbox{}
  ...
  Lists available shell commands in this instance of the WRPC.

\item[init add <cmd>] \mbox{}

  \hypertarget{cmd init add}{}
  Adds shell command at the end of the initialization script.

\item[init boot] \mbox{}

  Executes the script stored in Flash/EEPROM (the same
  action is done automatically when WRPC starts after resetting the soft-core
  processor).

\item[init erase] \mbox{}

  Erases the initialization script in Flash/EEPROM.

\item[init show] \mbox{}

  Prints all commands from the script stored in Flash/EEPROM.

\item[ip get]
\item[ip set <ip>] \mbox{}

  Reports or sets the IPv4 address of the WRPC (only available if
  \texttt{CONFIG\_IP} is set at build time.

\item[ip stat] \mbox{}

  Disp packet usage per client (only available if
  \texttt{CMD\_IP\_STAT} is set at build time.

\item[leapsec]
\item[leapsec get] \mbox{}

  Get number of leap seconds from PTP and from the system.

\item[leapsec set <num>] \mbox{}

  Set number of leap seconds for PTP. This settings make sense only
  for GM mode.

\item[ltest]
\item[ltest fake <nsecs>] \mbox{}

  Fakes delay to trigger latency failures (for testing).

\item[ltest <secs> <msecs>] \mbox{}

  Reads or sets the latency-test sending interval. See \ref{Latency Test}.
  Available if \texttt{CONFIG\_LATENCY\_PROBE} is set.

\item[ltest quiet] \mbox{}

  Disables sending latency messages to Syslog.

\item[ltest verbose] \mbox{}

  Enables sending latency messages to Syslog.

\item[mac getp] \mbox{}

  Reads the persistent MAC address stored in Flash/EEPROM.

\item[mac get] \mbox{}

  Prints WRPC's MAC address.

\item[mac setp <mac>] \mbox{}

  Stores the persistent MAC address in Flash/EEPROM.

\item[mac set <mac>] \mbox{}

  Sets the MAC address of WRPC.

\item[mode gm|master|slave] (legacy) \mbox{}

  This command is now an alias of the \texttt{ptp} command.

\item[netconsole] \mbox{}

  Prints peer's MAC, IP and port number used by the netconsole; to
  enable \code{netconsole} command \texttt{CONFIG\_CMD\_NETCONSOLE}
  has to be set in the dot-config; for more information about
  netconsole please refer to the section \ref{Netconsole}.

\item[netconsole <MAC> <IP> <PORT>] \mbox{}

  Sets peer's MAC, IP and port number for the netconsole.

\item[netconsole wait] \mbox{}

  Let the netconsole to wait for communication from peer; netconsole
  will transmit when the next netconsole's packet arrives to WRPC.

\item[netconsole disable] \mbox{}

  Disable netconsole until enabled manually.

\item[pll cl <channel>] \mbox{}

  Checks if SoftPLL is locked for the channel.

\item[pll gdac <index>] \mbox{}

  Gets dac's value.

\item[pll gps <channel>] \mbox{}

  Gets current and target phase shift for the channel.

\item[pll init <mode> <ref\_channel> <align\_pps>] \mbox{}

  Manually runs \texttt{spll\_init()} function to initialize SoftPll.

\item[pll sdac <index> <val>] \mbox{}

  Sets the dac.

\item[pll sps <channel> <picoseconds>] \mbox{}

  Sets phase shift for the channel.

\item[pll start <channel>] \mbox{}

  Starts SoftPLL for the channel.

\item[pll stop <channel>] \mbox{}

  Stops SoftPLL for the channel.

\item[pps] \mbox{}

  Check status of a PPS generation independent of PPSI state.

\item[pps force <on|off>] \mbox{}

  Enable/disable PPS generation independent of PPSI state. It is
  usefull mostly for testing purposes.

\item[ps] \mbox{}

  Prints the list of running tasks (processes) in the CPU. For each
  task you get the number of iterations, the maximum execution time
  (measured with the monotonic clock) and the CPU time consumed (using
  the RT clock) since boot or last reset of values.

\item[ps reset] \mbox{}

  Zeroes the profiling information reported by the \code{ps} command.

\item[ps max <msecs>] \mbox{}

  Starts printing all tasks executing longer than a given number of
  miliseconds. Additionally, it triggers printing messages if
  particular task runs longer than ever before. Passing ``\code{0}''
  as a parameter stops the further printouts.

\item[ptp <e2e|p2p>] \mbox{}

  Selects PTP delay mechanism: end-to-end or peer-to-peer.  If
  configured, you can set \texttt{p2p} mode. Alternatively you can use
  also aliases: \texttt{delay} (instead of \texttt{e2e}) or
  \texttt{pdelay} (instead of \texttt{p2p}).

\item[ptp gm|master|slave] \mbox{}

  Sets WRPC to operate as Grandmaster clock (requires external 10MHz
  and 1-PPS reference), Master or Slave. After setting the mode,
  \texttt{ptp start} must be re-issued.

\item[ptp abscal] \mbox{}

  Sets WRPC for absolute calibration.  You need special hardware for
  this process, which is described in
  \url{https://gitlab.com/ohwr/project/wr-calibration/wikis}.

\item[ptp <accuracy|allan|class|domain|egress|ingress|prio1|prio2|tsource> <val>] \mbox{}

  Sets various parametes of PTP. For details see below.  Use these
  subcommands with caution. Setting the mode with \texttt{ptp} command
  may affect the already set parameters using \texttt{ptp} subcommands
  listed below.  Consider using \texttt{ptp master} and \texttt{ptp start}
  before using mentioned subcommands.

\item[ptp prio1 <0-255>] \mbox{}

  Sets \textit{priority 1} used by PTP.

\item[ptp prio2 <0-255>] \mbox{}

  Sets \textit{priority 2} used by PTP.

\item[ptp domain <0-255>] \mbox{}

  Sets \textit{domain numbner} used by PTP.
\item[ptp class <1-255>] \mbox{}

  Sets \textit{clock class} used by PTP.

\item[ptp accuracy <1-255>] \mbox{}

  Sets \textit{clock accuracy} used by PTP.

\item[ptp allan <0-65535>] \mbox{}

  Sets \textit{clock allan variance} used by PTP.

\item[ptp tsource <0-255>] \mbox{}

  Sets \textit{time source} used by PTP.

\item[ptp egress <int>] \mbox{}

  Sets in runtime \textit{Slave PHY delay TX} used by PTP.
  If the change is small enough servo will stay in TRACK\_PHASE.

\item[ptp egress <int>] \mbox{}

  Sets in runtime \textit{Slave PHY delay RX} used by PTP.
  If the change is small enough servo will stay in TRACK\_PHASE.

\item[ptp start] \mbox{}

  Starts WR PTP daemon.

\item[ptp stop] \mbox{}

  Stops WR PTP daemon.

\item[ptp <cmd> <cmd> ...] \mbox{}

  You can concatenate several of the above subcommands in a single
  \texttt{ptp} command. With no arguments, the command reports the
  current values.

\item[ptrack <enable|disable>] \mbox{}

  Print status, enable or disable phase tracking. Useful for demos.

\item[refresh] \mbox{}

  Changes the update time period of the gui and stat commands.
  Default period is 1 second. If you set the period to 0, the log
...
\item[sdb fs <device> <baseadr> <param>] \mbox{}

  \hypertarget{cmd sdb fs}{} Creates SDBFS image under specified
  \code{<baseadr>} in selected storage depending on \code{<device>}
  ({\bf 0} - Flash, {\bf 1} - I2C EEPROM, {\bf 2} - 1-Wire
  EEPROM). The meaning of last parameter \code{<param>} depends on the
  type of selected storage. It is either the sector size in kilobytes
  (for Flash) or I2C chip address (for I2C EEPROM). Command \code{sdb}
  is available if \texttt{CONFIG\_GENSDBFS} is set.

\item[sdb fs 0] \mbox{}

  Creates SDBFS image in Flash memory. Base address and sector size
  are taken from HDL Syscon registers for SPEC/SVEC boards. If you
  want to use it for custom board, base address and sector size must
  be specified as VHDL generic parameters of the WR PTP Core. Command
  \code{sdb} is available if \texttt{CONFIG\_GENSDBFS} is set.

\item[sdb fse <device> <baseadr> <param>] \mbox{}

  \hypertarget{cmd sdb fse}{} Erases SDBFS image under specified
  \code{<baseadr>} from selected storage depending on \code{<device>}
  ({\bf 0} - Flash, {\bf 1} - I2C EEPROM, {\bf 2} - 1-Wire
  EEPROM). The meaning of last parameter \code{<param>} depends on the
  type of selected storage. It is either the sector size in kilobytes
  (for Flash) or I2C chip address (for I2C EEPROM). Command \code{sdb}
...
\item[sdb fse 0] \mbox{}
  ...
  Erases SDBFS image from Flash memory. Base address and sector size
  are taken from HDL Syscon registers for SPEC/SVEC boards. If you
  want to use it for custom board, base address and sector size must
  be specified as VHDL generic parameters of the WR PTP Core. Command
  \code{sdb} is available if \texttt{CONFIG\_GENSDBFS} is set.

\item[sfp add <PN> <deltaTx> <deltaRx> <alphaH> <alphaL>] \mbox{}

  Stores calibration parameters for SFP to a file in
  Flash/EEPROM. \code{<alphaL>} is the 9 lower decimal digits of alpha
  value, \code{<alphaH>} are the 9 higher decimal digits. For negative
  alpha values only \code{<alphaH>} is negative.

\item[sfp erase] \mbox{}

  Erases the SFP database stored in the Flash/EEPROM.

\item[sfp info] \mbox{}

  Prints detailed information about the plugged SFP.
  Command is available if \texttt{CONFIG\_CMD\_SFP\_INFO} is set.
  Can show monitoring data like temperature, RX and TX power if supported
  (available if \texttt{CONFIG\_SFP\_DOM} is set).
  For tunable SFP's another additional set of parameters is displayed like:
  supported channels, current channel or wavelength.

\item[sfp match] \mbox{}

  Prints the ID of a currently used SFP transceiver and tries to load
  the calibration parameters for it.

\item[sfp tune <channel|wavelength>] \mbox{}

  Tunes the TX of a plugged SFP. Command can take \textit{channel}
  (SFP channel, not an ITU channel!) or \textit{wavelength} in nm.
  Available if \texttt{CONFIG\_CMD\_SFP\_TUNABLE} is set.

\item[sfp show] \mbox{}

  Prints all SFP transceivers stored in database.

\item[stat <on|off>] \mbox{}

  Without parameter, toggles reporting of loggable statistics. You can
  pass \texttt{on} or \texttt{off} as argument as an alternative to
  toggling.

\item[stat bts] \mbox{}

  Prints bitslide value for established WR Link, needed by the
  calibration procedure.

\item[syslog off]

\item[syslog <ipaddr> <macaddr>] \mbox{}

  Disables or sets your \textit{syslog} server. See
  \ref{Syslog}. Available if \texttt{CONFIG\_SYSLOG} is set.
...
\item[time raw] \mbox{}
  ...
  Prints current time in a raw format (seconds, nanoseconds).

\item[time setnsec <nsec>] \mbox{}

  Sets only nanoseconds of the WRPC time.

\item[time setsec <sec>] \mbox{}

  Sets only seconds of the WRPC time (useful for setting time in
  GrandMaster mode, when nanoseconds counter is aligned to external
  1-PPS and 10 MHz).

\item[time set <sec> <nsec>] \mbox{}

  Sets WRPC time.

\item[verbose <digits>] \mbox{}

  Sets PPSi verbosity. See the PPSi manual about the meaning of the
  digits (hint: \texttt{verbose 1111} is a good first bet to see how
  the PTP system is working).

\item[ver] \mbox{}

  Prints which version of wrpc is running.

\item[vlan] \mbox{}

\item[vlan set <n>] \mbox{}

  Reports or sets the active vlan used for sending/receiving network
  frames. The command exists only if \texttt{CONFIG\_VLAN} is set.

\item[vlan off] \mbox{}

  Disables vlans. Available if \texttt{CONFIG\_VLAN} is set.

\item[w1] \mbox{}

  Lists onewire devices on the bus.

\item[w1w <offset> <byte> [<byte> ...]] \mbox{}

\item[w1r <offset> <len>] \mbox{}

  If \texttt{CONFIG\_W1} is set and a OneWire EEPROM exists, write and
  read data. For writing, \texttt{byte} values are decimal.
...
First, please download the SDBFS image from \textit{https://gitlab.com}:
\begin{lstlisting}
$ wget https://gitlab.com/ohwr/project/wr-cores/wikis/uploads/\
6f33b5a8660bd0f2ff7a07526b4484d6/sdbfs-flash.bin
\end{lstlisting}
...
\begin{lstlisting}
$ wget https://gitlab.com/ohwr/project/wr-cores/wikis/uploads/\
9dad995e74a3d49b472b8654833be01a/sdbfs-standalone-160812.bin
\end{lstlisting}
...
\label{appendix:wrpc_dump_with_Older_WRPC_Binaries}
...
\label{appendix:adding_new_objects_to_the_SNMP}
...
% --------------------------------------------------------------------------
\newpage
\section{Building RISC-V toolchain}
\label{appendix:building_RISCV_toolchain}

This section describes how to build from scratch the toolchain for RISC-V.
