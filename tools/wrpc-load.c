...
{
	wrc_writel (desc, 0xb00 + WRC_CPU_CSR_RESET, rst);
}

static void wrc_write_uaddr(struct mapping_desc *desc, unsigned int addr)
{
	wrc_writel(desc, 0xb00 + WRC_CPU_CSR_UADDR, addr >> 2);
}
                         ...
                         unsigned addr)
{
	...
	while (len > 0) {
		...
		v = (buf[3] << 0)
			...
			| (buf[1] << 16)
			| (buf[0] << 24);
		wrc_writel(desc, 0xb00 + WRC_CPU_CSR_UDATA, v);
		...
		if (verbose)
			printf ("Write %08x at %08x\n", v, addr);
                ...
                if (flag_check) {
                        uint32_t r;
                        wrc_write_uaddr(desc, addr);
                        r = wrc_readl(desc, 0xb00 + WRC_CPU_CSR_UDATA);
                        if (r != v) {
                                printf ("Error at %08x: "
...
static int wrc_save_firmware(struct mapping_desc *desc, const char *filename)
{
	...
	for (addr = 0; addr < length;) {
		...
		for (off = 0; off < l; off += 4) {
			unsigned int v;
			wrc_write_uaddr(desc, addr);
			v = wrc_readl(desc, 0xb00 + WRC_CPU_CSR_UDATA);
			/* Use BE */
			buf[off + 0] = v >> 24;
			buf[off + 1] = v >> 16;
			buf[off + 2] = v >> 8;
